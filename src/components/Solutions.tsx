...
const Solutions = () => {
  const handleDemo = (solutionType: string) => {
    switch (solutionType) {
      case 'voice':
        alert('Voice Recognition Demo: Click "Allow" to enable microphone access and start speaking in Hindi, Tamil, or Bengali!');
        break;
      case 'chatbot':
        alert('Multilingual Chatbot Demo: Try asking "मुझे बैंक बैलेंस चेक करना है" or "எனது கணக்கு நிலுவை என்ன?"');
        break;
      case 'translation':
        alert('Translation API Demo: Text will be translated from English to regional languages in real-time!');
        break;
      case 'ocr':
        alert('OCR Demo: Upload a document in Devanagari, Tamil, or Bengali script for instant digitization!');
        break;
      case 'education':
        alert('Educational Tools Demo: Practice pronunciation of "नमस्ते" (Namaste) and get AI feedback!');
        break;
      default:
        break;
    }
  ...
  const handleScheduleDemo = () => {
    alert('Demo Scheduled! Our team will contact you within 24 hours to set up a personalized demonstration.');
  };
      ...
      <div className="container mx-auto px-4">
          ...
          {solutions.map((solution, index) => (
            <Card key={index} className="hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-white border-0 shadow-lg">
              ...
              <CardContent className="text-center">
                ...
                <Button 
                  onClick={() => handleDemo(solution.demoType)}
        ...
        <div className="text-center mt-16">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" onClick={handleGetStarted} className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600">
              Get Started Free
            </Button>
            <Button size="lg" variant="outline" onClick={handleScheduleDemo} className="border-orange-500 text-orange-600 hover:bg-orange-50">
              Schedule Demo
            </Button>
          </div>
        </div>
